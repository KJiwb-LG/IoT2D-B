...
async function fetchTempData(date) {
    try {
        ...
        let highestTemp = findMax(sampledData.map(entry => entry.y));
        let lowestTemp = findMin(sampledData.map(entry => entry.y));
        let averageTemp = findAverage(sampledData.map(entry => entry.y));

        console.log("highestTemp:", highestTemp, "  lowestTemp:", lowestTemp, "  avg: ", averageTemp);
        ...
        myLineChart.update();
        document.getElementById('highestTemp').textContent = 'Highest temperature: ' + highestTemp + '°C';  
        document.getElementById('lowestTemp').textContent = 'Lowest temperature: ' + lowestTemp + '°C';
        document.getElementById('averageTemp').textContent = 'Average temperature: ' + averageTemp + '°C';
