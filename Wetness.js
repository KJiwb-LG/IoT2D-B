...
// 定义一个异步函数，用于获取JSON数据
async function fetchWetData(date) {
    try {
        // 读取并解析JSON文件
        ...
        let highestWet = findMax(sampledData.map(entry => entry.y));
        let lowestWet = findMin(sampledData.map(entry => entry.y));
        let averageWet = findAverage(sampledData.map(entry => entry.y));

        console.log("highestWet:", highestWet, "  lowestWet:", lowestWet, "  avg: ", averageWet);
        ...
        myLineChart.update();
        document.getElementById('highestWet').textContent = 'Highest humidity: ' + highestWet + '%';  
        document.getElementById('lowestWet').textContent = 'Lowest humidity: ' + lowestWet + '%';
        document.getElementById('averageWet').textContent = 'Average humidity: ' + averageWet + '%';
